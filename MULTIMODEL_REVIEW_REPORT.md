...
その後 Fable 5 が再び使えるようになったため、この gateway では `claude-fable-5` を Claude OAuth pass-through alias として追加しました。結論としては、Sonnet 5 は Fable 5 の完全な代替というより、実務 executor としての安定枠です。日本語の勝負文書や難しい設計判断は Fable / Opus 系、普段の作業は Sonnet 5、別 vendor の精査は Codex、という整理が現実的です。
...
- Fable 5: 日本語の勝負文書、自然な対話、Opus 4.6 系の延長としての重めの判断
